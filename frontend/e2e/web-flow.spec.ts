...
async function runGuestChat(page: Page): Promise<void> {
  ...
  await expect(page.getByText(/游客使用确定性 Fake/)).toBeVisible();
...
async function runRegisteredFlow(page: Page): Promise<void> {
  ...
  await page.getByRole("button", { name: "保存 Mock 支付" }).click();
  await expect(page.getByText(/当前：¥129\.90/)).toBeVisible();
  ...
  await expect(chatRegion.getByText(/Mock 退款已完成并验证/)).toBeVisible();
  ...
  await expect(l2Case.getByText("l2_resolved")).toBeVisible();
  await expect(l2Case.getByText(/get_order/)).toBeVisible();
  await expect(l2Case.getByText(/get_shipment/)).toBeVisible();
  await expect(l2Case.getByText(/上下文 \d+\/\d+ 项/)).toBeVisible();

  await page.reload();
  const restoredL2Case = page.getByRole("complementary", {
    name: "AI 二线 Case 状态",
  });
  await expect(restoredL2Case.getByText("l2_resolved")).toBeVisible();
  await expect(restoredL2Case.getByText(/get_order/)).toBeVisible();
  await expect(restoredL2Case.getByText(/get_shipment/)).toBeVisible();
  const businessBasis = restoredL2Case
    .locator("details")
    .filter({ hasText: "business_observation" })
    .first();
  await businessBasis.locator("summary").click();
  await expect(businessBasis.getByText(/business_observation/)).toBeVisible();

  await page.getByRole("link", { name: "订单" }).click();
  await expect(
    page.locator("li").filter({ hasText: "¥129.90" }).filter({ hasText: "succeeded" }),
  ).toBeVisible();
}
