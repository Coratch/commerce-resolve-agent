...
def _registered_conversation(web_harness: WebHarness) -> tuple[str, dict[str, str]]:
    """创建注册用户、私有订单和活动会话，返回 thread 与写请求 Header。"""

    session = web_harness.register_and_login(f"v06.{uuid4().hex[:8]}")
    ...
    conversation = web_harness.client.post("/api/conversations", headers=headers)
    assert conversation.status_code == 201
    return str(conversation.json()["thread_id"]), headers
...
def test_async_run_persists_messages_and_replays_sse(
    ...
    thread_id, headers = _registered_conversation(web_harness)
    order_id = web_harness.client.get("/api/orders").json()["orders"][0]["order_id"]
